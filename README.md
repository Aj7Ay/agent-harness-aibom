...
[![Python](https://img.shields.io/pypi/pyversions/agent-harness-aibom.svg)](https://pypi.org/project/agent-harness-aibom/)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)

**An AI Bill of Materials (AIBOM) generator, security analyzer, and
CI gate for AI agent harnesses.**

Point it at a running [Hermes](https://github.com/Aj7Ay) or
[OpenClaw](https://docs.openclaw.ai) installation and it answers the
...
line to stderr explaining what it skipped, so the resulting document is
...
were generated by running `scan` against the fixtures in
`tests/fixtures/` and are schema-validated against the real CycloneDX 1.6
JSON Schema on every test run. Their `report` renderings are
[`examples/hermes-aibom.example.html`](examples/hermes-aibom.example.html)
...
- `.github/workflows/ci.yml` runs the test suite (including real,
...
`src/harness_aibom/__init__.py`, commit, push, then publish a GitHub
Release with a matching tag (e.g. `v0.9.0`). The release triggers
`publish.yml`, which builds and uploads it automatically.
...
`test_cli.py` run against a real, locally installed `cosign` binary and
skip themselves (never mock it) when one isn't on `PATH`.
